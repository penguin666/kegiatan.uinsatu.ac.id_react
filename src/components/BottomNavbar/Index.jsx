...
function Index() {
    ...
    const [active, setActive] = useState(
        menus.findIndex((x) => x.link === pathname || (x.link !== "/" && pathname.startsWith(x.link)))
    );

    useEffect(() => {
        setActive(menus.findIndex((x) => x.link === pathname || (x.link !== "/" && pathname.startsWith(x.link))));
    }, [pathname, menus]);

    return (
        <div className="md:hidden print:hidden flex justify-center fixed bottom-0 w-full bg-white left-0 z-20 h-16">
            <div className="flex text-gray-600 relative">
                ...
                {menus.map((menu, index) => (
                    <NavLink
                        key={index}
                        to={menu.link}
                        onClick={menu.label === "Keluar" ? handleLogout : () => setActive(index)}
