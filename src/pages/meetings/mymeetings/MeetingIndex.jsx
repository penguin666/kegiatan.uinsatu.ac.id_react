...
function MeetingIndex() {
    ...
    const fetchData = async () => {
        setDoadingData(true)
        try {
            const result = await get(`/my-meetings?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleOnView = (item) => {
        return `/meetings/my-meetings/${item.id}`;
    }
    ...
    const handleSubmit = async (newData) => {
        try {
            ...
            if (editData) {
                result = await put(`/my-meetings/${editData.id}`, accessToken, newData);
            } else {
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/my-meetings/${itemToDelete?.id}`, accessToken, {});
    ...
    if (!hasPermission('create mymeetings'))
    {
        return <NotFound />
    }
        ...
        <>
            <div className="container mx-auto px-4 mt-5">
                <div className="flex justify-between items-center mb-5">
                    <div>
                        <h2 className="text-xl">Rapat Saya</h2>
                    </div>
