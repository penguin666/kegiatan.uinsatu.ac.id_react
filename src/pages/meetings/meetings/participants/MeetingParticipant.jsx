...
export default function MeetingParticipant() {
    ...
    const handleIsComingClick = async (item) => {
        try {
            const result = await put(`/meetings/${meeting.id}/participants/${item.id}`, accessToken, {});
    ...
    const fetchParticipants = async () => {
        setLoadingData(true)
        try {
            const result = await get(`/meetings/${meeting.id}/participants?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleSubmit = async (selected) => {
        try {
            const result = await post(`/meetings/${meeting.id}/participants`, accessToken, {user_id:selected});
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/meetings/${meeting.id}/participants/${itemToDelete.id}`, accessToken, {});
