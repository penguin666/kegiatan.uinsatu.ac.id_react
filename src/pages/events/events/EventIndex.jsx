...
function EventIndex() {
    ...
    const fetchData = async () => {
        setDoadingData(true)
        try {
            const result = await get(`/events?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleSubmit = async (newData) => {
        try {
            ...
            if (editData) {
                result = await put(`/events/${editData.id}`, accessToken, newData);
            } else {
                result = await post('/events', accessToken, newData);
            }
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/events/${itemToDelete?.id}`, accessToken, {});
        ...
        <>
            <div className="container mx-auto px-4 mt-5">
                <div className="flex justify-between items-center mb-5">
                    <div>
                        <h2 className="text-xl">Kegiatan Admin</h2>
                    </div>
