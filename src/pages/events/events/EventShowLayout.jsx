...
export default function EventShowLayout() {
    ...
    const downloadExcel = async () => {
        try {
            const response = await axios.get(`${BASE_API_URL}/events/${eventId}/participants/download`, {
                responseType: 'blob',
                headers: {
    ...
    const handleOptionSelect = (option) => {
        ...
        {
            window.open(`/events/admin/${eventId}/print/attendees`,'_blank', 'rel=noopener noreferrer')
        }
        else if (option.value === 'cvnsp')
        {
            window.open(`/events/admin/${eventId}/print`,'_blank', 'rel=noopener noreferrer')
        }
                    ...
                    <div className="col-span-2">
                        <div className="justify-center space-x-4 grid grid-cols-2">
                            ...
                            <div className="bg-teal-700 shadow rounded-lg overflow-hidden text-white mb-3">
                                <div className="px-3 py-2">
                                    <Link
                                        to={`/events/admin/${eventId}/sessions`}
                                        className="flex flex-row justify-center items-center text-gray-200 hover:text-white transition"
                                    >
