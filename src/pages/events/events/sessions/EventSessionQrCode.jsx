...
export default function EventSessionQrCode({session, uniqueCode})
{
    ...
    const generateQrCode = async () => {
        setLoading(true)
        try {
            const result = await post(`/events/${session.event_id}/sessions/${session.id}/generate-code`, accessToken, {});
    ...
    const refreshQrCode = async () => {
        setLoading(true)
        try {
            const result = await get(`/events/${session.event_id}/sessions/${session.id}`, accessToken, {});
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <div className="p-4 flex justify-between border-b">
                                <h2 className="text-lg font-bold text-center">QR Code Sesi {session.name} Waktu Presensi {moment(session.start_time).format('D MMMM Y HH:mm')} s.d. {moment(session.end_time).format('D MMMM Y HH:mm')}</h2>
                                <button
