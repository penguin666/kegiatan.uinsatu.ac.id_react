...
export default function EventSession() {
    ...
    const fetchSessions = async () => {
        setLoadingData(true)
        try {
            const result = await get(`/events/${event.id}/sessions?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleOnView = (item) => {
        return `/events/admin/${event.id}/sessions/${item.id}`;
    }
    ...
    const handleSubmit = async (formData) => {
        try {
            ...
            if (editData) {
                result = await put(`/events/${event.id}/sessions/${editData.id}`, accessToken, formData);
            } else {
                result = await post(`/events/${event.id}/sessions`, accessToken, formData);
            }
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/events/${event.id}/sessions/${itemToDelete.id}`, accessToken, {});
    ...
    return (
        <>
            ...
            <Modal
                ...
                loading={loading}
            >
                <p>Semua peserta yang sudah presensi akan dihapus. Apakah anda yakin ingin menghapus
                    Sesi {itemToDelete?.name}?</p>
            </Modal>
        </>
    )
}