...
import GuestTypeFormModal from "./GuestTypeFormModal.jsx";
...
function GuestTypeIndex() {
    ...
    const fetchData = async () => {
        setLoadingData(true)
        try {
            const result = await get(`/guest-types?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleSubmit = async (newData) => {
        try {
            ...
            if (editData) {
                result = await put(`/guest-types/${editData.id}`, accessToken, newData);
            } else {
                result = await post('/guest-types', accessToken, newData);
            }
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/guest-types/${itemToDelete?.id}`, accessToken, {});
    ...
    if (!hasPermission('create guesttypes'))
    {
        return <NotFound />
    }
        ...
        <>
            <div className="container mx-auto px-4 mt-5">
                <div className="flex justify-between items-center mb-5">
                    <div>
                        <h2 className="text-xl">Jenis Tamu</h2>
                    </div>
