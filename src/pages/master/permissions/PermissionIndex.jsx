...
function EventIndex() {
    ...
    const fetchData = async () => {
        setLoadingData(true)
        try {
            const result = await get(`/permissions?page=${page}&size=${rowsPerPage}&sortBy=${sorting.sortBy}&sortOrder=${sorting.sortOrder}&term=${search}`, accessToken, {});
    ...
    const handleSubmit = async (newData) => {
        try {
            ...
            if (editData) {
                result = await put(`/permissions/${editData.id}`, accessToken, newData);
            } else {
                result = await post('/permissions', accessToken, newData);
            }
    ...
    const handleConfirmed = async () => {
        setLoading(true)
        try {
            const result = await del(`/permissions/${itemToDelete?.id}`, accessToken, {});
        ...
        <>
            <div className="container mx-auto px-4 mt-5">
                <div className="flex justify-between items-center mb-5">
                    <div>
                        <h2 className="text-xl">Perizinan</h2>
                    </div>
            ...
            <Modal
                ...
                loading={loading}
            >
                <p>Hapus perizinan {itemToDelete?.name}?</p>
            </Modal>
